...
{
   public final String TARGET_ENDPOINT_ADDRESS = "http://" + getServerHost() + ":8080/jbossws-samples-handler";

   private static HeaderTestService port;

   public static Test suite()
   {
      return JBossWSTestSetup.newTestSetup(HeaderClientTestCase.class, "jbossws-samples-handler.war, jbossws-samples-handler-client.jar");
   }
   ...
   protected void setUp() throws Exception
   {
      ...
      if (port == null)
      {
         ...
         else
         {
            ...
            URL wsdlURL = new File("resources/samples/handler/WEB-INF/wsdl/TestService.wsdl").toURL();
            URL mappingURL = new File("resources/samples/handler/WEB-INF/jaxrpc-mapping.xml").toURL();
            QName qname = new QName("http://org.jboss.ws/samples/handler", "TestService");
            Service service = factory.createService(wsdlURL, qname, mappingURL);
            port = (HeaderTestService)service.getPort(HeaderTestService.class);
            ((Stub)port)._setProperty(Stub.ENDPOINT_ADDRESS_PROPERTY, "http://" + getServerHost() + ":8080/jbossws-samples-handler");
         }
      }
   }
