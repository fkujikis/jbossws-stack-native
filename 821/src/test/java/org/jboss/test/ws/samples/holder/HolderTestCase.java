...
public class HolderTestCase extends JBossWSTest
{
   ...
   {
      return JBossWSTestSetup.newTestSetup(HolderDIITestCase.class, "jbossws-samples-holder.war, jbossws-samples-holder-client.jar");
   }
   ...
   protected void setUp() throws Exception
   {
      ...
      if (port == null)
      {
         ...
         else
         {
            ...
            URL wsdlURL = new File("resources/samples/holder/WEB-INF/wsdl/TestService.wsdl").toURL();
            URL mappingURL = new File("resources/samples/holder/WEB-INF/jaxrpc-mapping.xml").toURL();
            QName qname = new QName("http://org.jboss.ws/samples/holder", "TestService");
            Service service = factory.createService(wsdlURL, qname, mappingURL);
            port = (HolderTestService)service.getPort(HolderTestService.class);
            ((Stub)port)._setProperty(Stub.ENDPOINT_ADDRESS_PROPERTY, "http://" + getServerHost() + ":8080/jbossws-samples-holder");
         }
      }
   }
